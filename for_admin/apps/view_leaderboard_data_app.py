...
st.write("このアプリでは、データストアに登録されているリーダーボードデータを閲覧できます。")

if st.button("リーダーボードデータを表示"):
    st.info(f"'{config.DATA_STORE_TYPE}' データストアからリーダーボードデータを読み込んでいます...")
    ...
    try:
        ...
        if df.empty:
            ...
            st.success(f"データストアから {len(df)} 件のリーダーボードデータを読み込みました。")
...
st.markdown("---")
st.write("ヒント: 参加者が提出を行い、スコアが計算されるとリーダーボードデータが更新されます。")
